...
@section('title', trans('admin.tour-slide.actions.index'))

@section('body')

    <tour-slide-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/tour-slides') }}'" inline-template>

        <div class="row">
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.tour-slide.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                            href="{{ url('admin/tour-slides/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                            {{ trans('admin.tour-slide.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' width="5%" data-title="id" :column="'id'">
                                            {{ trans('admin.tour-slide.columns.id') }}</th>
                                        <th is='sortable' width="15%" data-title="title" :column="'title'">
                                            {{ trans('admin.tour-slide.columns.title') }}</th>
                                        <th is='sortable' width="15%" data-title="description" :column="'description'">
                                            {{ trans('admin.tour-slide.columns.description') }}</th>
                                        <th is='sortable' width="15%" data-title="tour_id" :column="'tour_id'">
                                            {{ trans('admin.tour-slide.columns.tour_id') }}</th>
                                        <th is='sortable' width="15%" data-title="type" :column="'type'">
                                            {{ trans('admin.tour-slide.columns.type') }}</th>
                                        <th is='sortable' width="15%" data-title="created_at" :column="'created_at'">
                                            {{ trans('admin.tour-slide.columns.created_at') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="7">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/tour-slides')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/tour-slides/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="tour_id">@{{ item.tour.title }}</td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/tour-slides/create') }}"
