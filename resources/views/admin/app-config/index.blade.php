...
@section('title', trans('admin.app-config.actions.index'))

@section('body')

    <app-config-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/app-configs') }}'"
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.app-config.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                            href="{{ url('admin/app-configs/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                            {{ trans('admin.app-config.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover ">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' data-title="id" :column="'id'">
                                            {{ trans('admin.app-config.columns.id') }}</th>
                                        <th is='sortable' data-title="key" :column="'key'">
                                            {{ trans('admin.app-config.columns.key') }}</th>
                                        <th is='sortable' data-title="value_json" :column="'value_json'">
                                            {{ trans('admin.app-config.columns.value_json') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="7">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/app-configs')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/app-configs/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td class="bulk-checkbox" data-title="id">
                                        ...
                                        <td data-title="value_json">@{{ item.value_json }}</td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/app-configs/create') }}"
