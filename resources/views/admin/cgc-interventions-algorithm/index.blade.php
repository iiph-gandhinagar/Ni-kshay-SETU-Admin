...
@section('title', trans('admin.cgc-interventions-algorithm.actions.index'))

@section('body')

    <cgc-interventions-algorithm-listing :data="{{ $data->toJson() }}" :all_cadres="{{ json_encode($all_cadres) }}"
        :all_states="{{ json_encode($state) }}"
        :url="'{{ url('admin/cgc-interventions-algorithms?master=' . request()->query('master') . '&master_node_id=' . request()->query('master_node_id')) }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.cgc-interventions-algorithm.actions.index') }}
                        @can('admin.cgc-interventions-algorithm.create')
                            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                                href="{{ url('admin/cgc-interventions-algorithms/create') }}?master={{ app('request')->input('master') }}&master_node_id={{ app('request')->input('master_node_id') }}"
                                role="button"><i class="fa fa-plus"></i>&nbsp;
                                {{ trans('admin.cgc-interventions-algorithm.actions.create') }}</a>
                        @endcan
                        <a class="btn btn-danger btn-spinner btn-sm pull-right m-b-0 mr-1" href="#"
                            ...
                            {{ trans('admin.cgc-interventions-algorithm.actions.back') }}</a>
                    </div>
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' width="5%" data-title="id" :column="'id'">
                                            {{ trans('admin.cgc-interventions-algorithm.columns.id') }}</th>
                                        <th is='sortable' width="15%" data-title="title" :column="'title'">
                                            {{ trans('admin.cgc-interventions-algorithm.columns.title') }}</th>
                                        <th is='sortable' width="5%" data-title="node_type" :column="'node_type'">
                                            {{ trans('admin.cgc-interventions-algorithm.columns.node_type') }}</th>
                                        <th is='sortable' width="5%" data-title="is_expandable"
                                            :column="'is_expandable'">
                                            {{ trans('admin.cgc-interventions-algorithm.columns.is_expandable') }}</th>
                                        <th is='sortable' width="5%" data-title="has_options" :column="'has_options'">
                                            {{ trans('admin.cgc-interventions-algorithm.columns.has_options') }}</th>
                                        <th is='sortable' width="5%" data-title="parent_id" :column="'parent_id'">
                                            {{ trans('admin.cgc-interventions-algorithm.columns.parent_id') }}</th>
                                        <th is='sortable' width="5%" data-title="master_node_id"
                                            :column="'master_node_id'">
                                            {{ trans('admin.cgc-interventions-algorithm.columns.master_node_id') }}</th>
                                        <th is='sortable' width="5%" data-title="cadre_id" :column="'cadre_id'">
                                            {{ trans('admin.diagnoses-algorithm.columns.cadre_id') }}</th>
                                        <th is='sortable' width="5%" data-title="state_id" :column="'state_id'">
                                            {{ trans('admin.diagnoses-algorithm.columns.state_id') }}</th>
                                        {{-- <th is='sortable' :column="'description'">{{ trans('admin.cgc-interventions-algorithm.columns.description') }}</th> --}}
                                        <th is='sortable' width="5%" data-title="time_spent" :column="'time_spent'">
                                            {{ trans('admin.cgc-interventions-algorithm.columns.time_spent') }}</th>
                                        <th is='sortable' width="5%" data-title="index" :column="'index'">
                                            {{ trans('admin.cgc-interventions-algorithm.columns.index') }}</th>
                                        {{-- <th is='sortable' :column="'redirect_algo_type'">{{ trans('admin.cgc-interventions-algorithm.columns.redirect_algo_type') }}</th>
                                        <th is='sortable' :column="'redirect_node_id'">{{ trans('admin.cgc-interventions-algorithm.columns.redirect_node_id') }}</th>
                                        <th is='sortable' :column="'header'">{{ trans('admin.cgc-interventions-algorithm.columns.header') }}</th>
                                        <th is='sortable' :column="'sub_header'">{{ trans('admin.cgc-interventions-algorithm.columns.sub_header') }}</th> --}}
                                        <th is='sortable' width="5%" data-title="activated" :column="'activated'">
                                            {{ trans('admin.cgc-interventions-algorithm.columns.activated') }}</th>
                                        <th width="20%"></th>
                                    </tr>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="16">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/cgc-interventions-algorithms')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/cgc-interventions-algorithms/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                    ...
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="title">
                                            <span
                                                ...
                                                <span v-if="item.parent_id == 0">
                                                    <a
                                                        :href="'/admin/cgc-interventions-algorithms?master=' + item.id +
                                        ...
                                        <td class="action_buttons">
                                            <div class="row">
                                                ...
                                                @can('admin.cgc-interventions-algorithm.edit')
                                                    <div class="col-auto">
                                                ...
                                                @endcan
                                                @can('admin.cgc-interventions-algorithm.delete')
    ...
    </cgc-interventions-algorithm-listing>
