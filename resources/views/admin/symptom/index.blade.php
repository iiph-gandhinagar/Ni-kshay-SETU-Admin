...
@section('title', trans('admin.symptom.actions.index'))

@section('body')

    <symptom-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/symptoms') }}'"
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.symptom.actions.index') }}
                        @can('admin.symptom.create')
                            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                                href="{{ url('admin/symptoms/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                                {{ trans('admin.symptom.actions.create') }}</a>
                        @endcan
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover ">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' data-title="id" :column="'id'">
                                            {{ trans('admin.symptom.columns.id') }}</th>
                                        <th is='sortable' data-title="category" :column="'category'">
                                            {{ trans('admin.symptom.columns.category') }}</th>
                                        <th is='sortable' data-title="symptoms_title" :column="'symptoms_title'">
                                            {{ trans('admin.symptom.columns.symptoms_title') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/symptoms')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/symptoms/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="symptoms_title">@{{ item.symptoms_title }}</td>

                                        <td class="action_buttons">
                                            <div class="row">
                                                @can('admin.symptom.edit')
                                                    <div class="col-auto">
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                @can('admin.symptom.create')
                                    <a class="btn btn-primary btn-spinner" href="{{ url('admin/symptoms/create') }}"
