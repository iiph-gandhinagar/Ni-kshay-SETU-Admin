...
@section('title', trans('admin.cadre.actions.index'))

@section('body')

    <cadre-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/cadres') }}'" :session_search="'{{ $search }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.cadre.actions.index') }}
                        @can('admin.cadre.create')
                            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('admin/cadres/create') }}"
                                role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.cadre.actions.create') }}</a>
                        @endcan
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' data-title="id" :column="'id'">
                                            {{ trans('admin.cadre.columns.id') }}</th>
                                        <th is='sortable' data-title="title" :column="'title'">
                                            {{ trans('admin.cadre.columns.title') }}</th>
                                        <th is='sortable' data-title="cadre_type" :column="'cadre_type'">
                                            {{ trans('admin.cadre.columns.cadre_type') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="4">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/cadres')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/cadres/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                @can('admin.cadre.create')
                                    <a class="btn btn-primary btn-spinner" href="{{ url('admin/cadres/create') }}"
                                        role="button"><i class="fa fa-plus"></i>&nbsp;
                                        {{ trans('admin.cadre.actions.create') }}</a>
                                @endcan
                            </div>
                        </div>
                    </div>
