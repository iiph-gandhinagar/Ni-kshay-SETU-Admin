...
@section('title', trans('admin.app-management-flag.actions.index'))

@section('body')

    <app-management-flag-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/app-management-flags') }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.app-management-flag.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                            href="{{ url('admin/app-management-flags/create') }}" role="button"><i
                                class="fa fa-plus"></i>&nbsp; {{ trans('admin.app-management-flag.actions.create') }}</a>
                    </div>
                        ...
                        <div class="card-block">
                            ...
                            <table class="table table-hover ">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' data-title="id" :column="'id'">
                                            {{ trans('admin.app-management-flag.columns.id') }}</th>
                                        <th is='sortable' data-title="variable" :column="'variable'">
                                            {{ trans('admin.app-management-flag.columns.variable') }}</th>
                                        <th is='sortable' data-title="value" :column="'value'">
                                            {{ trans('admin.app-management-flag.columns.value') }}</th>
                                        <th is='sortable' data-title="type" :column="'type'">
                                            {{ trans('admin.app-management-flag.columns.type') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/app-management-flags')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/app-management-flags/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        {{-- <t d>@{{ item.id }}</td> --}}
                                        <td data-title="variable">@{{ item.variable }}</td>
                                        <td data-title="value">@{{ item.value }}</td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner"
                                    href="{{ url('admin/app-management-flags/create') }}" role="button"><i
                                        class="fa fa-plus"></i>&nbsp;
                                    {{ trans('admin.app-management-flag.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
