                    ...
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover ">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' data-title="id" :column="'id'">Id</th>
                                        <th is='sortable' data-title="permission_id" :column="'permission_id'">{{ trans('admin.role-has-permission.columns.permission_id') }}</th>
                                        <th is='sortable' data-title="role_id" :column="'role_id'">{{ trans('admin.role-has-permission.columns.role_id') }}</th>
                                    ...
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="4">
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3" @click="bulkDelete('/admin/role-has-permissions/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id" :class="bulkItems[item.id] ? 'bg-bulk' : ''">
                                        ...
                                        <td data-title="permission_id" >@{{ item.permission.name }}</td>
                                        <td data-title="role_id" >@{{ item.role.name }}</td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/role-has-permissions/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.role-has-permission.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
