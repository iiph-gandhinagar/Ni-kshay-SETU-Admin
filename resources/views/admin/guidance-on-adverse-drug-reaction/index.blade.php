...
@section('title', trans('admin.guidance-on-adverse-drug-reaction.actions.index'))

@section('body')

    <guidance-on-adverse-drug-reaction-listing :data="{{ $data->toJson() }}" :all_cadres="{{ json_encode($all_cadres) }}"
        :all_states="{{ json_encode($state) }}"
        :url="'{{ url('admin/guidance-on-adverse-drug-reactions?master=' . request()->query('master') . '&master_node_id=' . request()->query('master_node_id')) }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        ...
                        {{ trans('admin.guidance-on-adverse-drug-reaction.actions.index') }}
                        @can('admin.guidance-on-adverse-drug-reaction.create')
                            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                                href="{{ url('admin/guidance-on-adverse-drug-reactions/create') }}?master={{ app('request')->input('master') }}&master_node_id={{ app('request')->input('master_node_id') }}"
                                role="button"><i class="fa fa-plus"></i>&nbsp;
                                {{ trans('admin.guidance-on-adverse-drug-reaction.actions.create') }}</a>
                        @endcan
                        <a class="btn btn-danger btn-spinner btn-sm pull-right m-b-0 mr-1" href="#"
                            ...
                            {{ trans('admin.guidance-on-adverse-drug-reaction.actions.back') }}
                        </a>
                    </div>
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' width="5%" data-title="id" :column="'id'">
                                            {{ trans('admin.guidance-on-adverse-drug-reaction.columns.id') }}</th>
                                        <th is='sortable' width="15%"data-title="title" :column="'title'">
                                            {{ trans('admin.guidance-on-adverse-drug-reaction.columns.title') }}</th>
                                        <th is='sortable' width="5%" data-title="node_type" :column="'node_type'">
                                            {{ trans('admin.guidance-on-adverse-drug-reaction.columns.node_type') }}</th>
                                        ...
                                        <th is='sortable' width="5%" data-title="has_options" :column="'has_options'">
                                            {{ trans('admin.guidance-on-adverse-drug-reaction.columns.has_options') }}</th>
                                        <th is='sortable' width="5%" data-title="parent_id" :column="'parent_id'">
                                            {{ trans('admin.guidance-on-adverse-drug-reaction.columns.parent_id') }}</th>
                                        <th is='sortable' width="5%" data-title="master_node_id"
                                            :column="'master_node_id'">
                                            {{ trans('admin.guidance-on-adverse-drug-reaction.columns.master_node_id') }}
                                        </th>
                                        <th is='sortable' width="5%" data-title="cadre_id" :column="'cadre_id'">
                                            {{ trans('admin.guidance-on-adverse-drug-reaction.columns.cadre_id') }}</th>
                                        <th is='sortable' width="5%" data-title="state_id" :column="'state_id'">
                                            {{ trans('admin.guidance-on-adverse-drug-reaction.columns.state_id') }}</th>
                                        <th is='sortable' width="5%" data-title="index" :column="'index'">
                                            {{ trans('admin.guidance-on-adverse-drug-reaction.columns.index') }}</th>
                                        <th is='sortable' width="5%" data-title="time_spent" :column="'time_spent'">
                                            {{ trans('admin.guidance-on-adverse-drug-reaction.columns.time_spent') }}</th>
                                        {{-- <th is='sortable' :column="'description'">{{ trans('admin.guidance-on-adverse-drug-reaction.columns.description') }}</th>
                                        <th is='sortable' :column="'redirect_algo_type'">{{ trans('admin.guidance-on-adverse-drug-reaction.columns.redirect_algo_type') }}</th>
                                        <th is='sortable' :column="'redirect_node_id'">{{ trans('admin.guidance-on-adverse-drug-reaction.columns.redirect_node_id') }}</th>
                                        <th is='sortable' :column="'header'">{{ trans('admin.guidance-on-adverse-drug-reaction.columns.header') }}</th>
                                        <th is='sortable' :column="'sub_header'">{{ trans('admin.guidance-on-adverse-drug-reaction.columns.sub_header') }}</th> --}}
                                        <th is='sortable' width="5%" data-title="activated" :column="'activated'">
                                            {{ trans('admin.guidance-on-adverse-drug-reaction.columns.activated') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="16">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/guidance-on-adverse-drug-reactions')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/guidance-on-adverse-drug-reactions/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                    ...
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="title">
                                            <span
                                                ...
                                                <span v-if="item.parent_id == 0">
                                                    <a
                                                        :href="'/admin/guidance-on-adverse-drug-reactions?master=' + item.id +
                                        ...
                                        <td class="action_buttons">
                                            <div class="row">
                                                ...
                                                @can('admin.guidance-on-adverse-drug-reaction.edit')
                                                    <div class="col-auto">
                                                ...
                                                @endcan
                                                @can('admin.guidance-on-adverse-drug-reaction.delete')
    ...
    </guidance-on-adverse-drug-reaction-listing>
