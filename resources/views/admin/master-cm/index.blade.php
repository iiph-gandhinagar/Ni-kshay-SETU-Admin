...
@section('title', trans('admin.master-cm.actions.index'))

@section('body')

    <master-cm-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/master-cms') }}'" inline-template>

        <div class="row">
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.master-cm.actions.index') }}
                        @can('admin.master-cm.create')
                            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                                href="{{ url('admin/master-cms/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                                {{ trans('admin.master-cm.actions.create') }}</a>
                        @endcan
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th width="5%" is='sortable' data-title="id" :column="'id'">
                                            {{ trans('admin.master-cm.columns.id') }}</th>
                                        <th width="30%" is='sortable' data-title="title" :column="'title'">
                                            {{ trans('admin.master-cm.columns.title') }}</th>
                                        <th width="45%" is='sortable' data-title="description" :column="'description'">
                                            {{ trans('admin.master-cm.columns.description') }}</th>

                                        <th width="15%"></th>
                                    </tr>
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td width="5%" class="bg-bulk-info d-table-cell text-center" colspan="5">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/master-cms')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/master-cms/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                @can('admin.master-cm.create')
                                    <a class="btn btn-primary btn-spinner" href="{{ url('admin/master-cms/create') }}"
