...
@section('title', trans('admin.static-release.actions.index'))

@section('body')

    <static-release-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/static-releases') }}'"
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.static-release.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                            href="{{ url('admin/static-releases/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                            {{ trans('admin.static-release.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover ">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' width="5%" data-title="id" :column="'id'">
                                            {{ trans('admin.static-release.columns.id') }}</th>
                                        <th is='sortable' width="25%" data-title="features" :column="'features'">
                                            {{ trans('admin.static-release.columns.features') }}</th>
                                        <th is='sortable' width="25%" data-title="bugs_fix" :column="'bugs_fix'">
                                            {{ trans('admin.static-release.columns.bugs_fix') }}</th>
                                        <th is='sortable' width="10%" data-title="date" :column="'date'">
                                            {{ trans('admin.static-release.columns.date') }}</th>
                                        <th is='sortable' width="5%" data-title="order_index" :column="'order_index'">
                                            {{ trans('admin.static-release.columns.order_index') }}</th>
                                        <th is='sortable' width="5%" data-title="active" :column="'active'">
                                            {{ trans('admin.static-release.columns.active') }}</th>
                                        <th is='sortable' width="10%" data-title="created_at" :column="'created_at'">
                                            {{ trans('admin.static-release.columns.created_at') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="8">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/static-releases')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/static-releases/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="features">@{{ item.features }}</td>
                                        {{-- <td>
                                            <span v-for="(kw, i) in getFeatures(item.features)" class="badge badge-warning m-1"
                                                style="font-size: 0.8rem;">@{{ kw }} </span>
                                        </td> --}}
                                        <td data-title="bugs_fix">@{{ item.bugs_fix }}</td>
                                        <td data-title="date">@{{ item.date | moment }}</td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/static-releases/create') }}"
