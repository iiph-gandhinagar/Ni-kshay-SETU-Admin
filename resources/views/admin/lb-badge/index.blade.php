...
@section('title', trans('admin.lb-badge.actions.index'))

@section('body')

    <lb-badge-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/lb-badges') }}'"
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.lb-badge.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                            href="{{ url('admin/lb-badges/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                            {{ trans('admin.lb-badge.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover ">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' width="10%" data-title="id" :column="'id'">
                                            {{ trans('admin.lb-badge.columns.id') }}</th>
                                        <th is='sortable' width="20%" data-title="level_id" :column="'level_id'">
                                            {{ trans('admin.lb-badge.columns.level_id') }}</th>
                                        <th is='sortable' width="20%" data-title="badge" :column="'badge'">
                                            {{ trans('admin.lb-badge.columns.badge') }}</th>
                                        <th is='sortable' width="20%" data-title="created_at" :column="'created_at'">
                                            {{ trans('admin.lb-badge.columns.created_at') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="5">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/lb-badges')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/lb-badges/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="level_id">@{{ item.lb_level.level }}</td>
                                        <td data-title="badge">@{{ item.badge }}</td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/lb-badges/create') }}"
