...
@section('title', trans('admin.differential-care-algorithm.actions.index'))

@section('body')

    <differential-care-algorithm-listing :data="{{ $data->toJson() }}" :all_cadres="{{ json_encode($all_cadres) }}"
        :all_states="{{ json_encode($state) }}"
        :url="'{{ url('admin/differential-care-algorithms?master=' . request()->query('master') . '&master_node_id=' . request()->query('master_node_id')) }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.differential-care-algorithm.actions.index') }}
                        @can('admin.differential-care-algorithm.create')
                            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                                href="{{ url('admin/differential-care-algorithms/create') }}?master={{ app('request')->input('master') }}&master_node_id={{ app('request')->input('master_node_id') }}"
                                role="button"><i class="fa fa-plus"></i>&nbsp;
                                {{ trans('admin.differential-care-algorithm.actions.create') }}</a>
                        @endcan
                        <a class="btn btn-danger btn-spinner btn-sm pull-right m-b-0 mr-1" href="#"
                            ...
                            {{ trans('admin.differential-care-algorithm.actions.back') }}
                        </a>
                    </div>
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' width="5%" data-title="id" :column="'id'">
                                            {{ trans('admin.differential-care-algorithm.columns.id') }}</th>
                                        <th is='sortable' width="15%"data-title="title" :column="'title'">
                                            {{ trans('admin.differential-care-algorithm.columns.title') }}</th>
                                        <th is='sortable' width="5%" data-title="node_type" :column="'node_type'">
                                            {{ trans('admin.differential-care-algorithm.columns.node_type') }}</th>
                                        <th is='sortable' width="5%" data-title="is_expandable"
                                            :column="'is_expandable'">
                                            {{ trans('admin.differential-care-algorithm.columns.is_expandable') }}</th>
                                        <th is='sortable' width="5%" data-title="has_options" :column="'has_options'">
                                            {{ trans('admin.differential-care-algorithm.columns.has_options') }}</th>
                                        <th is='sortable' width="5%" data-title="parent_id" :column="'parent_id'">
                                            {{ trans('admin.differential-care-algorithm.columns.parent_id') }}</th>
                                        <th is='sortable' width="5%" data-title="master_node_id"
                                            :column="'master_node_id'">
                                            {{ trans('admin.differential-care-algorithm.columns.master_node_id') }}</th>
                                        <th is='sortable' width="5%" data-title="cadre_id" :column="'cadre_id'">
                                            {{ trans('admin.differential-care-algorithm.columns.cadre_id') }}</th>
                                        <th is='sortable' width="5%" data-title="state_id" :column="'state_id'">
                                            {{ trans('admin.differential-care-algorithm.columns.state_id') }}</th>
                                        {{-- <th is='sortable' :column="'description'">{{ trans('admin.differential-care-algorithm.columns.description') }}</th> --}}
                                        <th is='sortable' width="5%" data-title="time_spent" :column="'time_spent'">
                                            {{ trans('admin.differential-care-algorithm.columns.time_spent') }}</th>
                                        <th is='sortable' width="5%" data-title="index" :column="'index'">
                                            {{ trans('admin.differential-care-algorithm.columns.index') }}</th>
                                        {{-- <th is='sortable' :column="'redirect_algo_type'">{{ trans('admin.differential-care-algorithm.columns.redirect_algo_type') }}</th>
                                        <th is='sortable' :column="'redirect_node_id'">{{ trans('admin.differential-care-algorithm.columns.redirect_node_id') }}</th>
                                        <th is='sortable' :column="'header'">{{ trans('admin.differential-care-algorithm.columns.header') }}</th>
                                        <th is='sortable' :column="'sub_header'">{{ trans('admin.differential-care-algorithm.columns.sub_header') }}</th> --}}
                                        <th is='sortable' width="5%" data-title="activated" :column="'activated'">
                                            {{ trans('admin.differential-care-algorithm.columns.activated') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="16">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/differential-care-algorithms')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/differential-care-algorithms/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                    ...
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="title">
                                            <span
                                                ...
                                                <span
                                                    v-if="item.node_type == 'Linking Node' || item.node_type == 'Linking Node Without Options'">
                                                    <a
                                                        :href="'/admin/differential-care-algorithms?master=' + item.id +
                                        ...
                                        <td class="action_buttons">
                                            <div class="row">
                                                ...
                                                @can('admin.differential-care-algorithm.edit')
                                                    <div class="col-auto">
                                                ...
                                                @endcan
                                                @can('admin.differential-care-algorithm.delete')
    ...
    </differential-care-algorithm-listing>
