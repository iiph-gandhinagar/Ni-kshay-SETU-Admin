...
const ChatbotQuestionsHitsCard = ({ }) => {
  ...
  useEffect(() => {
    try {
      setLoader(true)
      fetch(BASE_URL + `get-chatquestion-hit-count?type=${dateFilter}&state_id=${filters.state}&district_id=${filters.district}&block_id=${filters.block}&date=${filters.date}`).then((res) => res.json())
        .then((json) => {
          setData(json?.data)
          setLoader(false)
        }).catch((error) => {
    ...
    <div className="card h-100 mb-0">
      <div className="card-body p-0">
        <div className="d-flex align-items-center justify-content-between p-3 row p-0 m-0">
          <h5 className="section-title mb-0 col-12 col-sm-5 p-0 m-0">Chatbot Questions Hits</h5>
          <div className="d-flex align-items-center justify-content-end col-12 col-sm-7 p-0 m-0 mt-1 mt-sm-0">
          <button onClick={() => {
              window.location = `export-chatbot-question?state_id=${filters.state}&district_id=${filters.district}&block_id=${filters.block}&date=${filters.date}`
        ...
        <div className="d-flex align-items-center justify-content-between">
          <h6 className="card-tag px-2 text-end" style={{ width: 110, borderBottomRightRadius: 4, borderTopRightRadius: 4 }}>Questions</h6>
        ...
        {loader ?
          <div className="chatbot-list custom-scrollbar m-3">
            <ChatbotCardLoader id={"chatbot-list"} />
          </div> :
          <div className="chatbot-list custom-scrollbar m-3">
            {data.length > 0 ? data?.map((item, i) => {
              ...
              return (
                <div className="d-flex align-items-center justify-content-between chatbot-list-card me-xl-2 mr-0" key={"chatbot-list" + i}>
                  <h6 className="fs-16-Raleway mb-0 color-353535 col">{JSON.parse(item.question)?.en}</h6>
...
export default ChatbotQuestionsHitsCard;