...
@section('title', trans('admin.flash-similar-app.actions.index'))

@section('body')

    <flash-similar-app-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/flash-similar-apps') }}'" inline-template>

        <div class="row">
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.flash-similar-app.actions.index') }}
                        <a class="btn btn-primary btn-sm pull-right m-b-0 ml-2"
                            href="{{ url('admin/flash-similar-apps/export') }}" role="button"><i
                                class="fa fa-file-excel-o"></i>&nbsp;
                            {{ trans('admin.flash-similar-app.actions.export') }}</a>
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                            href="{{ url('admin/flash-similar-apps/create') }}" role="button"><i
                                class="fa fa-plus"></i>&nbsp; {{ trans('admin.flash-similar-app.actions.create') }}</a>
                    </div>
                        ...
                        <div class="card-block">
                            ...
                            <table class="table table-hover ">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' width="5%" data-title="id" :column="'id'">
                                            {{ trans('admin.flash-similar-app.columns.id') }}</th>
                                        <th is='sortable' width="15%" data-title="title" :column="'title'">
                                            {{ trans('admin.flash-similar-app.columns.title') }}</th>
                                        <th is='sortable' width="20%" data-title="sub_title" :column="'sub_title'">
                                            {{ trans('admin.flash-similar-app.columns.sub_title') }}</th>
                                        <th is='sortable' width="10%" data-title="href" :column="'href'">
                                            {{ trans('admin.flash-similar-app.columns.href') }}</th>
                                        <th is='sortable' width="5%" data-title="order_index" :column="'order_index'">
                                            {{ trans('admin.flash-similar-app.columns.order_index') }}</th>
                                        <th is='sortable' width="5%" data-title="active" :column="'active'">
                                            {{ trans('admin.flash-similar-app.columns.active') }}</th>
                                        <th is='sortable' width="15%" data-title="created_at" :column="'created_at'">
                                            {{ trans('admin.flash-similar-app.columns.created_at') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="10">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/flash-similar-apps')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/flash-similar-apps/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="sub_title">@{{ item.sub_title }}</td>
                                        <td data-title="href">@{{ item.href }}</td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner"
                                    href="{{ url('admin/flash-similar-apps/create') }}" role="button"><i
                                        class="fa fa-plus"></i>&nbsp;
                                    {{ trans('admin.flash-similar-app.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
