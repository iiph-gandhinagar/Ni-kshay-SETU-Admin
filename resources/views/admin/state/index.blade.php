...
@section('body')
    ...
    <state-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/states') }}'" :session_search="'{{ $search }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        ...
                        @can('admin.state.create')
                            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('admin/states/create') }}"
                                role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.state.actions.create') }}</a>
                        @endcan
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' data-title="id" :column="'id'">
                                            {{ trans('admin.state.columns.id') }}</th>
                                        <th is='sortable' data-title="country_id" :column="'country_id'">
                                            {{ trans('admin.state.columns.country_id') }}</th>
                                        <th is='sortable' data-title="title" :column="'title'">
                                            {{ trans('admin.state.columns.title') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="4">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/states')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/states/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                @can('admin.state.create')
                                    <a class="btn btn-primary btn-spinner" href="{{ url('admin/states/create') }}"
                                        role="button"><i class="fa fa-plus"></i>&nbsp;
                                        {{ trans('admin.state.actions.create') }}</a>
                                @endcan
                            </div>
                        </div>
                    </div>
