...
class AppNotOpened10Days extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app-not-opended:in-10-days';
    ...
    public function handle()
    {
        ...
        $not_opened_app_user = collect(array_merge($app_not_opened_user_for_10_days->toArray(), $user_15_days->toArray()))->unique()->values();
        $subscriber_10_days = Subscriber::whereNotIn('id', $not_opened_app_user)->pluck('id');
        //  Log::info("subscribers not opened app in 10 days ------------------------------------------>");
        // Log::info($subscriber_10_days);
        if (count($subscriber_10_days) > 0) {
            $notification['title'] = "App Not Opened";
            $notification['description'] = "It's been a while... your app is not opened within 10 days";

            $device_id = UserDeviceToken::whereIn('user_id', $subscriber_10_days)->get('notification_token');
