...
@section('title', trans('admin.diagnoses-algorithm.actions.index'))

@section('body')

    <diagnoses-algorithm-listing :data="{{ $data->toJson() }}" :all_cadres="{{ json_encode($all_cadres) }}"
        :all_states="{{ json_encode($state) }}"
        :url="'{{ url('admin/diagnoses-algorithms?master=' . request()->query('master') . '&master_node_id=' . request()->query('master_node_id')) }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.diagnoses-algorithm.actions.index') }}
                        @can('admin.diagnoses-algorithm.create')
                            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                                href="{{ url('admin/diagnoses-algorithms/create') }}?master={{ app('request')->input('master') }}&master_node_id={{ app('request')->input('master_node_id') }}"
                                role="button"><i class="fa fa-plus"></i>&nbsp;
                                {{ trans('admin.diagnoses-algorithm.actions.create') }}</a>
                        @endcan
                        <a class="btn btn-danger btn-spinner btn-sm pull-right m-b-0 mr-1" href="#"
                            ...
                            {{ trans('admin.diagnoses-algorithm.actions.back') }}</a>
                    </div>
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' width="5%" data-title="index" :column="'index'">
                                            {{ trans('admin.diagnoses-algorithm.columns.index') }}</th>
                                        <th is='sortable' width="5%" data-title="time_spent" :column="'time_spent'">
                                            {{ trans('admin.diagnoses-algorithm.columns.time_spent') }}</th>
                                        {{-- <th is='sortable' :column="'description'">{{ trans('admin.diagnoses-algorithm.columns.description') }}</th>
                                        <th is='sortable' :column="'redirect_algo_type'">{{ trans('admin.diagnoses-algorithm.columns.redirect_algo_type') }}</th>
                                        <th is='sortable' :column="'redirect_node_id'">{{ trans('admin.diagnoses-algorithm.columns.redirect_node_id') }}</th>
                                        <th is='sortable' :column="'header'">{{ trans('admin.diagnoses-algorithm.columns.header') }}</th>
                                        <th is='sortable' :column="'sub_header'">{{ trans('admin.diagnoses-algorithm.columns.sub_header') }}</th> --}}
                                        <th is='sortable' width="5%" data-title="activated" :column="'activated'">
                                            {{ trans('admin.diagnoses-algorithm.columns.activated') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="16">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/diagnoses-algorithms')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/diagnoses-algorithms/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                    ...
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="title">
                                            <span
                                                ...
                                                <span v-if="item.parent_id == 0">
                                                    <a
                                                        :href="'/admin/diagnoses-algorithms?master=' + item.id +
                                        ...
                                        <td class="action_buttons">
                                            <div class="row">
                                                ...
                                                @can('admin.diagnoses-algorithm.edit')
                                                    <div class="col-auto">
                                                ...
                                                @endcan
                                                @can('admin.diagnoses-algorithm.delete')
