...
@section('body')
    ...
    <tour-listing :data="{{ $data->toJson() }}" :tour_slides="{{ json_encode($tour_slides) }}"
        :url="'{{ url('admin/tours') }}'" inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0" href="{{ url('admin/tours/create') }}"
                            role="button"><i class="fa fa-plus"></i>&nbsp; {{ trans('admin.tour.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' width="5%" data-title="id":column="'id'">
                                            {{ trans('admin.tour.columns.id') }}</th>
                                        <th is='sortable' width="15%" data-title="title" :column="'title'">
                                            {{ trans('admin.tour.columns.title') }}</th>
                                        <th is='sortable' width="5%" data-title="active":column="'active'">
                                            {{ trans('admin.tour.columns.active') }}</th>
                                        <th is='sortable' width="15%" data-title="default" :column="'default'">
                                            {{ trans('admin.tour.columns.default') }}</th>
                                        <th is='sortable' width="15%" data-title="created_at" :column="'created_at'">
                                            {{ trans('admin.tour.columns.created_at') }}</th>
                                        <th width="5%"></th>
                                    </tr>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/tours')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/tours/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="active">
                                            <span v-if="item.active == 1" class="badge badge-success m-1 p-1"
                                                style="color:white">Yes
                                            </span>
                                            <span v-else class="badge badge-danger m-1 p-1">No
                                        </td>
                                        <td data-title="default"><span v-if="item.default == 1"
                                                class="badge badge-success m-1 p-1" style="color:white">Yes
                                            </span>
                                            <span v-else class="badge badge-danger m-1 p-1">No
                                            </span>
                                        </td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/tours/create') }}"
                                    role="button"><i class="fa fa-plus"></i>&nbsp;
                                    {{ trans('admin.tour.actions.create') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
