...
@section('title', trans('admin.district.actions.index'))

@section('body')

    <district-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/districts') }}'"
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.district.actions.index') }}
                        @can('admin.district.create')
                            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                                href="{{ url('admin/districts/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                                {{ trans('admin.district.actions.create') }}</a>
                        @endcan
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' data-title="id" :column="'id'">
                                            {{ trans('admin.district.columns.id') }}</th>
                                        <th is='sortable' data-title="country_id" :column="'country_id'">
                                            {{ trans('admin.district.columns.country_id') }}</th>
                                        <th is='sortable' data-title="state_id" :column="'state_id'">
                                            {{ trans('admin.district.columns.state_id') }}</th>
                                        <th is='sortable' data-title="title" :column="'title'">
                                            {{ trans('admin.district.columns.title') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="5">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/districts')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/districts/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                @can('admin.district.create')
                                    <a class="btn btn-primary btn-spinner" href="{{ url('admin/districts/create') }}"
