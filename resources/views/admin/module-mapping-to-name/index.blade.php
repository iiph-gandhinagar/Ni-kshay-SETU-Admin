...
@section('title', trans('admin.module-mapping-to-name.actions.index'))

@section('body')

    <module-mapping-to-name-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/module-mapping-to-names') }}'"
        inline-template>
            ...
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.module-mapping-to-name.actions.index') }}
                        @can('admin.module-mapping-to-name.create')
                            <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                                href="{{ url('admin/module-mapping-to-names/create') }}" role="button"><i
                                    class="fa fa-plus"></i>&nbsp; {{ trans('admin.module-mapping-to-name.actions.create') }}</a>
                        @endcan
                    </div>
                            ...
                            <table class="table table-hover ">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' data-title="id" :column="'id'">
                                            {{ trans('admin.module-mapping-to-name.columns.id') }}</th>
                                        <th is='sortable' data-title="module_name" :column="'module_name'">
                                            {{ trans('admin.module-mapping-to-name.columns.module_name') }}</th>
                                        <th is='sortable' data-title="mapping_name" :column="'mapping_name'">
                                            {{ trans('admin.module-mapping-to-name.columns.mapping_name') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="5">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/module-mapping-to-names')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/module-mapping-to-names/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="module_name">@{{ item.module_name }}</td>
                                        <td data-title="mapping_name">@{{ item.mapping_name }}</td>

                                        <td class="action_buttons">
                                            <div class="row">
                                                @can('admin.module-mapping-to-name.edit')
                                                    <div class="col-auto">
                                                ...
                                                @endcan
                                                @can('admin.module-mapping-to-name.delete')
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                @can('admin.module-mapping-to-name.create')
                                    <a class="btn btn-primary btn-spinner"
                                        href="{{ url('admin/module-mapping-to-names/create') }}" role="button"><i
                                            class="fa fa-plus"></i>&nbsp;
                                        {{ trans('admin.module-mapping-to-name.actions.create') }}</a>
                                @endcan
                            </div>
                        </div>
                    </div>
    ...
    </module-mapping-to-name-listing>
