...
@section('title', trans('admin.lb-level.actions.index'))

@section('body')

    <lb-level-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/lb-levels') }}'" inline-template>

        <div class="row">
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.lb-level.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                            href="{{ url('admin/lb-levels/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                            {{ trans('admin.lb-level.actions.create') }}</a>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <table class="table table-hover ">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' width="5%" data-title="id" :column="'id'">
                                            {{ trans('admin.lb-level.columns.id') }}</th>
                                        <th is='sortable' width="25%" data-title="level" :column="'level'">
                                            {{ trans('admin.lb-level.columns.level') }}</th>
                                        <th is='sortable' width="25%" data-title="content" :column="'content'">
                                            {{ trans('admin.lb-level.columns.content') }}</th>
                                        <th is='sortable' width="15%" data-title="created_at" :column="'created_at'">
                                            {{ trans('admin.lb-level.columns.created_at') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="5">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/lb-levels')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/lb-levels/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="level">@{{ item.level }}</td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/lb-levels/create') }}"
