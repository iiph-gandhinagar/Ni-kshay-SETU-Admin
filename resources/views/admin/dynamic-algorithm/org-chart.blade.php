...
<script>
   (function($) {
  jQuery(function() {
   let masterIdString = location.search.split('&master=');
   let masterId = masterIdString[1];
   let key = masterIdString[0].split('?key=')[1];
   
   $.get(`/get-dynamic-algorithms-dependent-nodes/${key}/${masterId}`, function(data){
      var ds = data.data;
