...
@section('title', trans('admin.static-app-config.actions.index'))

@section('body')

    <static-app-config-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/static-app-configs') }}'" inline-template>

        <div class="row">
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> {{ trans('admin.static-app-config.actions.index') }}
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                            href="{{ url('admin/static-app-configs/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                            {{ trans('admin.static-app-config.actions.create') }}</a>
                    </div>
                        ...
                        <div class="card-block">
                            ...
                            <table class="table table-hover ">
                                <thead>
                                    <tr>
                                        ...
                                        <th is='sortable' width="5%" data-title="id" column="'id'">
                                            {{ trans('admin.static-app-config.columns.id') }}</th>
                                        <th is='sortable' width="15%" data-title="key" column="'key'">
                                            {{ trans('admin.static-app-config.columns.key') }}</th>
                                        <th is='sortable' width="15%" data-title="type" column="'type'">
                                            {{ trans('admin.static-app-config.columns.type') }}</th>
                                        <th is='sortable' width="15%" data-title="value_json" column="'value_json'">
                                            {{ trans('admin.static-app-config.columns.value_json') }}</th>
                                        <th is='sortable' width="15%" data-title="created_at" column="'created_at'">
                                            {{ trans('admin.static-app-config.columns.created_at') }}</th>
                                        ...
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="6">
                                            <span
                                                ...
                                                @{{ clickedBulkItemsCount }}. <a href="#" class="text-primary"
                                                    @click="onBulkItemsClickedAll('/admin/static-app-configs')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                    @click="bulkDelete('/admin/static-app-configs/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td data-title="type">@{{ item.type.toUpperCase() }}</td>
                                        <td data-title="value_json">
                                            <div :title=stripHTML(item.value_json)>@{{ item.value_json | stringCount | stripHTML }}<div>
                                        </td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner"
                                    href="{{ url('admin/static-app-configs/create') }}" role="button"><i
                                        class="fa fa-plus"></i>&nbsp;
                                    {{ trans('admin.static-app-config.actions.create') }}</a>
                            </div>
                        </div>
